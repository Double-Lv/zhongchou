(function($){
	$.popbox = function(opts){
		var defaults = {
			...
			], //默认显示的按钮，有：blue, gray, bigblue, biggreen
		...
		var box = {
			...
			init : function(template){
				...
				if(option.showBtn){
					...
					for(var i=0,len=option.btns.length; i<len; i++){
						var b = option.btns[i];
						var text = !!b.text ? b.text : (b.type=='blue' ? '确定' : '取消');
				...
				//点击确定按钮
				template.find('.pb_bluebtn').click(function(){
					var close = true;
				...
				//点击取消按钮
				template.find('.pb_graybtn').click(function(){
					var close = true;
				...
				//异步加载CSS
				!$.popbox.hasLoadedCss && (function(src, callback){
				...
				}('/resource/module/popbox/1.1/jquery.popbox.css',function(){
					$.popbox.hasLoadedCss = !0;
				}));
			...
			open : function(){
				...
				if(currentTemplate.length===0){
					...
					newbox.appendTo(option.parent||document.body).show();
				}
				else{
